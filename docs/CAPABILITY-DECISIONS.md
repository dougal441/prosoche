...
**Consequence for later phases:** Phase 5 builds CIRC-05 exactly as described above: Get Device Details read → has-any-value guard → `settings_snapshot` capture → Set Brightness at `safety.dim_target` (the bound this line originally named is **superseded by D-01** — see the **Supersession** note at the end of this entry) → per-run skip-to-message-only if the read fails or fails its numeric-sanity check. Phase 4's CLOSE handler (SESS-07) restores `settings_snapshot.brightness` whenever present, per `.planning/research/ARCHITECTURE.md` §9's four restoration triggers (owning CLOSE, Emergency Restore, cooldown-natural-expiry; never a superseded CLOSE). SAFE-05 (Emergency Restore) must include this same restore step. Under this decision, `state.json`'s `settings_snapshot` field holds, at minimum, a `brightness` key (`{ original_value, changed_at, changed_by_session_id }`) whenever a Dimming write is currently outstanding — populated only when CAP-17's read actually returned a usable value for that specific run, and cleared to `{}` once every outstanding key has been restored; a restore step never runs for a value that was never captured. See the consistency note at the end of BD-03.

**Addendum — floor corrected [Phase 9, experimental fork, 2026-08-16]. ⚠ Its *provisional* and *fork-scoped* framing is itself SUPERSEDED IN PLACE by D-01 — read the Supersession note that follows this paragraph; the finding below stands, only its status changed.** The floor clause in the Decision and Consequence paragraphs above — cited here and deliberately **not** restated, because a note that reproduces the clause it retires is itself a surviving occurrence of it — was written on the premise that `WFBrightness = 0.0` produces a literal, unusable black screen, and treated avoiding that value as itself a safety requirement. User-reported on-device observation corrects this: iOS's practical brightness minimum is dim, not black — the screen stays visible at the floor. This does not change the has-any-value guard or the restore mechanism, which were always the actual safety-load-bearing parts of this decision (see Rationale above — SAFE-03 is satisfied by capture-and-restore, not by floor avoidance). It changes only the target value: on this experimental fork, Dimming may target the device's true minimum rather than an artificial band, *contingent on Phase 9 device-proving the capture/restore loop* under real failure modes (force-quit, device restart, missed CLOSE, overlapping sessions) per `.planning/todos/pending/2026-08-16-reintroduce-and-validate-dimming-and-silence-stateful-restor.md`. This is a user report, recorded under the same evidence-hierarchy standard as the rest of this document: device ground truth first, but recorded and re-checked, not merely asserted.

*The two sentences that stood here are retired by D-01 and the reason each one goes is different.* They said the addendum was **provisional** and that it applied to **this branch only**, leaving the main line unaffected. Both were true when written and neither is true now: D-01 settled the floor **on the main line**, so the fork scoping is gone and the status is settled rather than provisional (Supersession note below).

**What did NOT change, and must not be read as having changed.** D-01 settles a *decision*; it proves nothing about *hardware*. Two things remain outstanding exactly as this addendum left them: the underlying observation still rests on **one unrepeated user report**, and the capture-and-restore loop is still **device-unproven** — phase 16 plan 01 made the capture durably persist before the device is changed, which is what makes the property satisfiable at all, but no run of it has ever happened on a phone. Both are tracked by this phase's UAT instrument and remain BLOCKED on DIST-03 (paired device present, `tunnelState: disconnected`, no live session to drive).

**Supersession — D-01, decided on the MAIN LINE [user decision, LOCKED 2026-08-17; recorded here 2026-08-18 by phase 16 plan 05]:**

`safety.brightness_floor` is `0` and `safety.dim_target` is `0` in both shipped forks. Three things follow, and each is stated separately because each has a different scope.

1. **BD-02's own historical band-and-floor text is superseded.** The Decision and Consequence paragraphs above named a bound; that bound is retired. It is **cited, never restated** — a supersession note that reproduces the clause it retires leaves a live false assertion sitting in the file, and `docs/retired_clause_check.py` reports it as one. The paragraphs are amended in place with pointers here rather than deleted, because the record of what was decided when is worth more than the space it costs.

2. **The Phase 9 addendum is promoted from provisional and fork-scoped to SETTLED on main.** What it observed is unchanged; what changed is its status. Its residual device-gated claims are restated immediately above it and are **not** settled by this note.

3. **Canonical strategy §21's floor clause is superseded on the main line by D-01, and this is the authority note the rest of the amended record depends on.** `PROSOCHE_Nine_Circles_Canonical_Strategy.md` is **retained unmodified** as the original design input — it is a historical design document, not a living spec, frozen by user decision 2026-08-18, and no task in phase 16 edits it. **Where the canonical strategy and BD-02 disagree on the brightness floor, BD-02 governs.** This matters concretely rather than formally: three amended sites elsewhere in the repo cite §21 as their authority and keep citing it — the two `safety` rows in `src/CONFIG-BLOCK.md`, CAP-16's Fallback cell in `docs/BUILD-NOTES.md`, and the capability-audit row for Set Brightness in `.claude/CLAUDE.md`. Without this note each of those reads as amended text pointing at an unamended authority, and the record would look consistent while being incoherent.

**On the §11 quotation in the Question paragraph.** That paragraph quotes canonical §11, a **frozen source**, and is retained as a quotation of one — with the floor sentence elided and the elision marked, so this document carries the citation without carrying the retired clause. Read the quotation through this note: the elided sentence is what D-01 superseded.

**The safety property is unchanged and was never the bound.** It is capture-and-restore reliability: the original brightness is captured **and durably persisted** before any change is applied, and is always restored; any setting whose original cannot be captured is left unchanged (SAFE-03). Avoiding a particular value was never itself the safety property. The durable-persistence half became true in phase 16 plan 01 — before it, the property could not have been satisfied by any build, which is what makes restating it here an honest correction rather than a weakening.

**Scope: brightness only.** BD-03's volume position is untouched — `safety.allow_volume_increase` stays `false` and Silence's Media-only scoping stands (SAFE-02). BD-01's Ash position is untouched.
