...
| CAP-20 parameters | `operation = turn` (string), `state = On` (bool), `ShowWhenRun = Off` | **Mixed shapes.** `operation` stays a **string** (`turn`\|`toggle`) — BD-01-R was right here. `state` is an **integer**: `on`=1, `off`=2 — not a bool. **No `ShowWhenRun` on the iOS intent.** | BD-01-R2 |
...
BD-01-R was right that Ash is real, right about why the catalog said otherwise, and right
that `operation` is a string enum case id. It was wrong about the **identifier** (it
prescribed the macOS `UA*` twin, which would import as an unavailable action) and about
**`state`**, which it typed as a bool — on iOS it is an integer, and the restore write is
`state = 2`, not `Off`. That is the failure mode that strands a user filtered. Corrected
from two donors plus Apple's own `AccessibilityUtilities.framework` intentdefinition.

**Restore leg not yet donor-confirmed.** `operation = "turn"` and `state = 2` rest on Apple's
schema, not on a device. BD-01-R2 gates Phase 5's CIRC-02 restore path on one further donor
built as "Turn Color Filters Off".
