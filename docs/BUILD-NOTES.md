...
| CAP-20 parameters | `operation = turn` (string), `state = On` (bool), `ShowWhenRun = Off` | **Integers.** `state`: `on`=1, `off`=2. `operation`: `turn`=1, `toggle`=2, elided when unset. **No `ShowWhenRun` on the iOS intent.** | BD-01-R2 |
...
BD-01-R was right that Ash is real and right about why the catalog said otherwise. It was
wrong about *what to build* — it prescribed the macOS identifier and a bool/string parameter
shape. Authoring that verbatim would have produced an unavailable action, and its restore
write (`state = Off`) would have been the wrong integer, the failure mode that strands a user
filtered. Corrected from the donor plus Apple's own `AccessibilityUtilities.framework`
intentdefinition.
