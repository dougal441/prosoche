...
| 2 | **`read_value()` / `get_value()` distinction** | Same numbered list or the adjacent runtime-semantics section | Already fully written as a docstring at `tools/build_state_engine.py:248-296` — the rule is: `get_value()` for COMPOUND values consumed as a List; `read_value()` for SCALARS in text/numeric comparisons. Guarded by `verify_compound_value_reads()`. **Corrected 2026-08-17 during 13-03 execution:** the `COMPOUND_STATE_KEYS` frozenset at `tools/build_state_engine.py:3503-3506` holds exactly `recent_sessions`, `recent_contracts`, `exit_events`, `profile_snapshot.enabled_exits` — this row originally omitted `recent_contracts` and listed `exit_stats.<n>.samples` as a member. It is not one: `exit_stats.<name>.samples` is a dynamic per-exit-type key built by `text_token()`, so the guard's literal-key scan cannot match it. The source records it *beside* the frozenset (`:3507-3511`) as the fifth confirmed instance of the axis so it is not lost from the class. Follow the source, not this table. |
