...
1. **Leaf naming: reuse `original_value`, or introduce an honest name?**
...
2. **Gate A's disposition — the phase's one genuinely open decision.**
...
3. **Can a rung-2 simulator probe say anything about the toggle at all?**
...
4. **Is the phase "done" without a device pass?**
...
5. **Does the emitted Shortcuts comment inside `ash()` need per-arm text?**
