...
## CLOSED 2026-08-17 — absorbed, no work remained here

Moved to `completed/` as a close-out only. This todo marked itself ABSORBED on 2026-08-16
and named its own successor; nothing in it was still actionable in place.

- **The experiment ran and won.** Phase 9 applied the numeric-coercion fix to all 28
  `setbrightness`/`setvolume` sites and merged to main (`2e2261e`), artifacts regenerated
  in `c6d8737`.
- **The opposing MVP cut is cancelled, permanently.** Phase 10 was re-planned on main
  specifically to exclude it, and `docs/environmental_restore_check.py` (added by Phase 10
  plan 10-03, commit `b18d415`) now *pins* `dimming()`, `silence()`,
  `restore_managed_settings()`, the `settings_snapshot` seed and both
  `NUMERIC_OPERAND_FIELDS` entries, so the cut can no longer be reintroduced by accident —
  the build fails if any of them is removed.
- **The outstanding device proof lives in the successor**,
  `2026-08-16-dimming-and-silence-as-distinct-circles.md`, together with the
  distinct-Circle sequence design and the revived DEV-06. `09-UAT.md`'s tests 2–12 are
  still the gate and are still unrun.

Retained above for its §21 reasoning and its enumeration of the failure modes the
successor's device testing still owes answers to.
