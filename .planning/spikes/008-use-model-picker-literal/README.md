...
Availability and runtime behaviour are untouched by this spike. Spike 004 already
established the safety posture on real ineligible hardware: `Use Model` fails with a
graceful native error and, because the core deterministic escalation is ordered first,
that failure costs only the bonus mirror text. This spike settles the **literal**, so the
Sentient branch can now be authored offline without a guessed picker value — it does not
relax the ordering requirement or the `ai_enabled` gate.
