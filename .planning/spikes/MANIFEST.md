...
| 011 | mirror-primitive-picker-discriminator | standard | Given the alert-free three-leg probe (List / Get Item From List / Speak Text, Circle 8's own index, `MIRROR_SUCCESSES` transcribed verbatim), when imported and run on the booted simulator with a bisection, then determine which of the three action identifiers unique to the Mirror primitive's span raises the device-reproduced axis-4 unfilled-picker error `"Please choose a value for each parameter in this action"`, or that none of them do at rung 2 | **PENDING** — Task 1 (build + sign) complete, gate A clean, shape mechanically asserted; Task 2 (simulator run + bisection + verdict) not yet executed | axis-4, unfilled-picker, mirror, voice, list, getitemfromlist, speaktext, simulator, rung-2, probe, evidence-hierarchy, bisection |
...
Spike `.planning/spikes/011-mirror-primitive-picker-discriminator/` (new, 2026-08-18) is built and
signed; its verdict is recorded once plan 15-02 Task 2 runs it on the simulator.
