...
- **Safety**: no unsafe or startling volume, no accessibility-stranding state, Emergency Restore always available. Every environmental change (brightness, volume) must be captured before it is applied and reliably restored — capture-and-restore reliability is the safety mechanism, not avoidance of the display's own minimum. **[Phase 9, experimental fork, 2026-08-16]** The prior absolute "never zero brightness" floor was corrected: iOS's practical brightness minimum is dim, not a literal black/unusable screen, per user on-device report — see `docs/CAPABILITY-DECISIONS.md` BD-02 addendum. This branch may dim to the device's true minimum, contingent on Phase 9 proving the capture/restore loop holds under real device failure modes (force-quit, restart, missed CLOSE, overlapping sessions); until that verdict lands, treat the relaxed floor as provisional, not confirmed. Main line is unaffected — this correction is scoped to this experimental fork only.
