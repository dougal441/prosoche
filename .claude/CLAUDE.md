...
PROSOCHĒ is a free iPhone Shortcut (source-available, free for noncommercial use) that restores the missing interval between the impulse to open a habit-forming app and the act of consuming it. It watches user-selected apps through native iOS Personal Automations (App Is Opened / Is Closed) and runs the **covenant model** on two axes: behavioural **Pressure** (Heat + Gravity) maps to nine Circles grouped into four fixed bands — Silent (0), Ambient (1–3), Ask (4–6), Rescue (7–9) — while a valid **intention contract** covers opens inside its window so they fire nothing at all. Silence is the reward for declared, bounded use; escalation is proportionate to behavioural evidence, in salience rather than frequency. It ships as two forks from one engine: **Core** (fully deterministic, broad iOS 26 support; source filename `PROSOCHE-Dumb.xml`) and **Aware** (same deterministic engine plus Apple's **On-Device** Intelligence model as an attention mirror and contract auditor inside a deterministic envelope; source filename `PROSOCHE-Sentient.xml`).
...
**Canonical spec:** `PROSOCHE_Nine_Circles_Canonical_Strategy.md` **v2.0** (2026-08-19, the covenant model; decisions BD-09..BD-12). v1.0 is preserved at git tag `pre-covenant-overhaul`, and canon v2 **Appendix A** maps every v1 `§N` to its v2 home — any historical "canonical strategy §N" citation in this file, `docs/BUILD-NOTES.md`, phase records, or decision records resolves through that table. **The shipped artifacts implement the v1 interaction model until conversion Phases 17–20 land** — checker docstrings and BUILD-NOTES describing the pre-menu, the v1 slot table, or Confession-era behaviour describe the current build, not the target design.
...
- **AI**: Apple Intelligence via the iOS 26 `Use Model` action, Aware fork only — On-Device pinned with the device-evidenced literal (BD-04-R2); PCC authorised fallback only; never ChatGPT; no model call on covered opens
- **Privacy**: no behavioural data leaves the device; Aware receives only a compact local context window, never the whole Note
...
- **Safety**: no unsafe or startling volume, no accessibility-stranding state, Emergency Restore always available and never gated on any Note-editable setting. Every environmental change (brightness, volume, Color Filters) must be captured **and durably persisted** before it is applied and reliably restored — or, for the two-valued Color Filters with no read-back, unconditionally set off on every recovery path — and any setting whose original cannot be captured is left unchanged. Capture-and-restore reliability is the safety mechanism, not avoidance of a particular value (**D-01**, LOCKED 2026-08-17: `safety.brightness_floor` and `safety.dim_target` are both `0` in both shipped forks; authority `docs/CAPABILITY-DECISIONS.md` BD-02). **Still outstanding:** the capture-and-restore loop is **device-unproven** — plan 16-01 made the capture persist before the device changes, but no run has happened on a phone, and the dim-not-black observation rests on one unrepeated user report. Both are owned by Phase 22's device UAT and remain BLOCKED on DIST-03. Do not read the settled decision as a settled device fact.
- **Determinism**: the model never controls arithmetic, thresholds, timers, coverage, verdict envelopes, Circle IX/Frozen, or any safety decision; all variability is counter-based and deterministic (BD-11) — `is.workflow.actions.number.random` appears nowhere
...
- Because each question targets one literal parameter field, the Layer-A import questions (canon v2 §15.1: the severity question mapping to Paradise/Purgatory/Inferno, the modality question mapping to a sequence, "May PROSOCHĒ speak? yes/no," and Aware's "Use on-device intelligence? yes/no") must each be implemented as: a **Text** action holding the literal default (e.g. `WFTextActionText = "Purgatory"`) targeted by one import question with `Text` = the prompt, whose output is then read at bootstrap-time and mapped/validated (e.g. an If-chain matching the answer text to a profile) rather than as a native yes/no or single-select import-time control. This is consistent with the canon's own instruction (v2 §15.1, v1 §7.1) to use `WFWorkflowImportQuestions` only for simple, robust parameters — the plist evidence shows exactly why: it's a literal-text-prefill mechanism, not a rich form-builder.
