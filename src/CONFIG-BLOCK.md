...
Only the Circle-6 entry above is unsettled. It is recorded here so a later reader cannot mistake that one interim stand-in for the designed behaviour.
...
**Note — the primitive named `Ash` in the two notes below now ships as `Black and White`.** BD-06 Decision 3 renamed it; plan 11-02 applied the rename to the sequence arrays and to the dispatch surface and changed **nothing** about its behaviour. Two sentences that stood here recorded that build's alert-only body and its Color-Filters-free artifact. Both were true when written and **both were retired by phase 14 plan 01**; they are cited by where they lived rather than restated. What is true now: `ash()` keeps its Python identifier and emits the real iOS Color Filters toggle; the alert is **deleted**, not kept as a fallback (D-14-C); and `docs/phase5_self_check.py` now asserts the iOS `AX*` identifier **present** at a derived count while keeping — deliberately, as a trap guard — its assertion that the macOS twin is absent. The `safety.ash_managed_color_filters` key keeps its name because it is a `state.json`-adjacent Config key, not a display name; phase 14 plan 01 made it live, and it now has its own row in `## Field reference` below.

**Note — SUPERSEDED TWICE, and true of the build as of 2026-08-19.** Ash — shipped as `Black and White` — **is** a real system Color Filters change on iOS. This note first asserted that on 2026-08-13 under BD-01-R, when it was true of **no build that existed**; it became true of the artifact at phase 14 plan 01, for a different reason, and that is recorded here rather than left to quietly come right. The governing capability record is `docs/CAPABILITY-DECISIONS.md` **BD-01-R2**, which rests on three decrypted device donors: the identifier is the iOS `AX*` intent under `AXSettingsShortcuts`, `state` is a bool-as-integer (`1` = On, `0` = Off), and `operation` is **omitted in both legs**. The identifier and the `operation` literal this note named before 2026-08-19 were the **macOS twin** and an elided default respectively; both are superseded by BD-01-R2, and both are cited by where they lived — this note's own pre-2026-08-19 text, and BD-01-R — rather than restated, because a supersession note that repeats what it supersedes is itself a surviving occurrence. Reaching for the macOS twin is the wrong fix for a red gate A: see `docs/BUILD-NOTES.md` DEV-08 and `docs/gate_a_residue_check.py`. `safety.ash_managed_color_filters` (default `true`) is the canonical-§21 opt-in; when `false` the Circle fires a bare Nothing — **not** BD-01's visual pause — per D-14-C and D-14-D. The paragraph below is retained for history; its claim that Ash is non-environmental no longer holds.
...
| `safety.brightness_floor` | `0` | Lower bound the Dimming write is clamped to. At `0` the device's own practical minimum is what the user sees — which iOS renders as dim, not as a black or unusable screen — and the safety of the primitive rests on the captured original being restored, not on this bound | §21, **superseded on the main line by D-01** (decided 2026-08-17, recorded 2026-08-18). The canonical strategy is retained unmodified as the original design input; `docs/CAPABILITY-DECISIONS.md` BD-02 is the authority where the two disagree | Must stay ≥ `0` and ≤ `safety.dim_target`. This is a tuning bound, not the safety mechanism: the safety property is that the original brightness is captured **and durably persisted** before any change and is always restored, which `docs/environmental_restore_check.py` pins structurally |
| `safety.dim_target` | `0` | Brightness value Dimming aims for. At `0` the write asks for the device's dimmest practical setting | §21, superseded on the main line by D-01 — see `docs/CAPABILITY-DECISIONS.md` BD-02, the governing authority | Must stay ≥ `safety.brightness_floor`, which `docs/environmental_restore_check.py` asserts. The two are equal at `0`, so the floor binds **exactly** rather than never — a floor below an unchanged target would have been inert |
...
| `safety.ash_managed_color_filters` | `true` | Whether `Black and White` may change the device's Color Filters. At `true` the Circle turns Color Filters **on**, and the unconditional off leg inside `restore_managed_settings()` turns them **off** again at all four recovery paths — the CLOSE pipeline, Emergency Restore, Ice expiry and the live-Ice redirect. At `false` the Circle fires a bare Nothing: no toggle, no alert, no notification, and PROSOCHĒ never touches Color Filters at all | §21's "require the user to opt into a known PROSOCHĒ-managed configuration" branch, via `docs/CAPABILITY-DECISIONS.md` **BD-01-R2**; dead Config until phase 14 plan 01 made it live | JSON boolean, so it reads back as numeric `1`/`0` — branch on `Is Greater Than 0`, never on a string compare to `"true"` (the same hazard as `allow_volume_increase`, above). Either value is valid; `true` is what ships. **There is no read-back for any accessibility setting on iOS**, so this flag is the only recourse a user who runs Color Filters themselves currently has — the Control Room Note discloses it in plain language, and the undone half is `.planning/todos/pending/2026-08-19-ash-void-circle-when-user-already-uses-grayscale.md`, which is **accepted and backlogged, not mitigated** |
...
- **2026-08-19** — Phase 14 plan 03 (the RECORD half of the phase; plan 14-01 was the BUILD half). **No key was added, removed or revalued: the fenced literal above is byte-identical to what it held before this plan, and `safety.ash_managed_color_filters` still ships `true`.** What changed is that this file stopped asserting two contradictory things about the `Black and White` primitive at once. Four sites were corrected, each in **every** asserting cell rather than only the value: the rename note's claim that the primitive's alert-only body was verbatim and that the phase-5 checker's no-Color-Filters assertion stayed green (plan 14-01 deleted the alert and **inverted** that assertion); the BD-01-R note's claim that the primitive was *already* a real Color Filters change (true of no build until plan 14-01, and now recorded as having come true for a different reason rather than left to come right quietly); the same note's macOS-twin identifier and elided `operation` literal, both superseded by **BD-01-R2**'s three decrypted device donors and now **cited by where they lived rather than restated**, because a supersession note that repeats what it supersedes is itself a surviving occurrence; and the `## Field reference` table, which had rows for both brightness keys and the volume key but **none** for the kill switch, despite that key being live in the fenced literal directly above it. A fifth site was found by an independent sweep of this file and corrected in the same pass: the header's cross-reference named **BD-01** as the decision shaping the `Ash` entry, when BD-01 and BD-01-R are both superseded and BD-01-R2 governs. The historical, twice-superseded BD-01 paragraph is **retained** with a pointer, per this file's own convention for a reversed decision. The Control Room Note literal in both built forks gained a plain-language disclosure of the Color Filters change and its kill switch in the same commit, edited through `tools/plist_text_edit.py`'s guarded round trip; this file is the fenced literal's mirror, not its source, and the Note is a separate preserved literal in the artifact.
- **2026-08-18** — plan 15-01 discharges Circle 8's interim declaration. `Loud Mirror` now dispatches the designed `voice()` primitive (distinct from `Mirror`'s `mirror()`); the preamble above was "Two entries here are deliberately INTERIM" and is now "One entry here is deliberately INTERIM" — only the Circle-6 `Eject`/`Redirect` stand-in remains. The Config JSON literal below is unchanged: the entry name `"Loud Mirror"` never moved, only the generator function it dispatches to.
- **2026-08-18** — Phase 16 plan 05 (the RECORD half of **D-01**). `safety.brightness_floor` `0.10 → 0` and `safety.dim_target` `0.12 → 0` in the fenced literal above, bringing this file back into agreement with what plan 16-03 shipped into both forks — this file's own header calls the fenced block "the transcription source, not a description of one", so a stale value here was a false statement about the build rather than a stale note. Both `safety` table rows were corrected in **every** asserting cell, not only the value: the purpose text, the cited authority and the constraint cell each carried an independent assertion of the retired bound. The §21 citations are **kept** and given a supersession pointer rather than removed — the canonical strategy is frozen as the original design input and is deliberately not edited; BD-02 is the authority where the two disagree. `safety.allow_volume_increase` and `safety.ash_managed_color_filters` are untouched: D-01 is brightness-only. `docs/retired_clause_check.py` (added by the same plan) now asserts mechanically that this fenced block and both built forks' Config literal agree on both keys, so the next drift of this kind is a failing check rather than a fifth enumeration pass.
